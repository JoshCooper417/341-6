...
__fresh253:
  ...
  %vdecl_slot834 = alloca i32
  store i32 -1, i32* %vdecl_slot834
  %lhs_or_call835 = load %A** %vdecl_slot833
  %cast_op836 = bitcast %A* %lhs_or_call835 to %C* 
  %cast_op837 = bitcast %C* %cast_op836 to i8** 
  %cast_op838 = bitcast %_C_vtable* @_C_vtable157 to i8* 
  %vt_ptr_slot839 = alloca i8*
  %tmp0841 = load i8** %cast_op837
  store i8* %tmp0841, i8** %vt_ptr_slot839
  br label %__loop252

__loop252:
  %tmp1842 = load i8** %vt_ptr_slot839
  %guard1846 = icmp eq i8* %tmp1842, %cast_op838
  br i1 %guard1846, label %__then249, label %__fall1251

__fresh254:
  br label %__fall1251

__fall1251:
  %guard2847 = icmp eq i8* %tmp1842, null
  br i1 %guard2847, label %__else248, label %__fall2250

__fresh255:
  br label %__fall2250

__fall2250:
  %tmp2843 = load i8** %vt_ptr_slot839
  %tmp3844 = bitcast i8* %tmp2843 to i8** 
  %tmp4845 = load i8** %tmp3844
  store i8* %tmp4845, i8** %vt_ptr_slot839
  br label %__loop252
...
__then249:
  %var_slot840 = alloca %C*
  store %C* %cast_op836, %C** %var_slot840
  call void @print_string( i8* @_const_str163 )
  %lhs_or_call851 = load %C** %var_slot840
  %path852 = getelementptr %C* %lhs_or_call851, i32 0, i32 2
  %lhs_or_call853 = load i32* %path852
  store i32 %lhs_or_call853, i32* %vdecl_slot834
  br label %__done247
...
__else248:
  ...
  %lhs_or_call848 = load %A** %vdecl_slot833
  %path849 = getelementptr %A* %lhs_or_call848, i32 0, i32 2
  %lhs_or_call850 = load i32* %path849
  store i32 %lhs_or_call850, i32* %vdecl_slot834
  br label %__done247

__done247:
  %lhs_or_call854 = load i32* %vdecl_slot834
  ret i32 %lhs_or_call854
}
